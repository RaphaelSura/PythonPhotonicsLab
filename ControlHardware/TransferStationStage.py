...
class Container(GridLayout):
    ...
    # stage related stuff
    cursor = 3
    position = [0.01, 0.1, 0.3, 1, 3, 10, 30, 100, 300, 1000]
    #stage = APTMotor(27253837, HWTYPE=31)
    ...
    def move_stage_forward(self):
        self.stage.mRel(-1 * float(self.display.text) / 1000)
        pass

    def move_stage_backward(self):
        self.stage.mRel(float(self.display.text) / 1000)
        pass


class TansferStationStageApp(App):

    def build(self):
        self.title = 'Transfer station Z stage'
        self.icon = "focus_app_icon.png"
        return Container()


TansferStationStageApp().run()
