...
# wrapper to make things easier to handle
class XPSstage(XPS):
    def __init__(self, ip, portnumber, groupnumber):
        XPS.__init__(self)
        # Connect to the XPS
        self.socketId = self.TCP_ConnectToServer(ip, portnumber, 20)
        # Check connection passed
        if self.socketId == -1:
            print('Connection to XPS failed, check IP & Port')

        self.group = 'Group' + str(groupnumber)
        self.positioner = self.group + '.Pos'
        self.current_pos = None

    def displayErrorAndClose(self, errorCode, APIName):
        if (errorCode != -2) and (errorCode != -108):
            [errorCode2, errorString] = self.ErrorStringGet(self.socketId, errorCode)
            if (errorCode2 != 0):
                print(APIName + ': ERROR ' + str(errorCode))
            else:
                print(APIName + ': ' + errorString.decode())
        else:
            if (errorCode == -2):
        ...
        def initialize(self):
            # Kill the group
            [errorCode, _] = self.GroupKill(self.socketId, self.group)  # getattr(myxps, 'GroupKill')(socketId, group)
            if errorCode != 0:
                self.displayErrorAndClose(errorCode, 'GroupKill')
                sys.exit()
            # Initialize the group
            [errorCode, _] = self.GroupInitialize(self.socketId, self.group)
            if errorCode != 0:
                self.displayErrorAndClose (errorCode, 'GroupInitialize')
                sys.exit()
            # Home search
            [errorCode, _] = self.GroupHomeSearch(self.socketId, self.group)
            if errorCode != 0:
    ...
    def shutdown(self):
        # Kill the group
        [errorCode, _] = self.GroupKill(self.socketId, self.group)
        if errorCode != 0:
            self.displayErrorAndClose(errorCode, 'GroupKill')
            sys.exit()
        # release the stage
    ...
    def move_by(self, dx):
        # relative motion of the stage by an amount dx millimeter
        [errorCode, _] = self.GroupMoveRelative(self.socketId, self.positioner, [dx])
        if errorCode != 0:
            self.displayErrorAndClose(errorCode, 'GroupMoveRelative')

    def move_to(self, xx):
        # absolute motion of the stage to xx millimeter
        [errorCode, _] = self.GroupMoveAbsolute(self.socketId, self.positioner, [xx])
        if errorCode != 0:
            self.displayErrorAndClose(errorCode, 'GroupMoveAbsolute')

    def read_position(self):
        # Get current position of the stage
        [errorCode, self.current_pos] = self.GroupPositionCurrentGet(self.socketId, self.positioner, 1)
        if errorCode != 0:
