...
class SpeFile:
    ...
    def _get_wavelength(self):
        ...
        try:
            ...
            try:
                wavelength_string = self.footer.SpeFormat.Calibrations.WavelengthMapping.WavelengthError.cdata

                wavelength=pd.read_csv(StringIO(wavelength_string), skiprows=0, delimiter=',', lineterminator=' ', header=None)
                return wavelength.values[:, 0]
            except AttributeError:
    ...
    def _get_exposure_time(self):
        """
        Returns exposure time contained in the xml footer
        """
        try:
            exp_time_str = self.footer.SpeFormat.DataHistories.DataHistory.Origin.Experiment.Devices.Cameras.Camera.ShutterTiming.ExposureTime
            exp_time = float(exp_time_str.cdata) / 1000
        except AttributeError:
            print("XML Footer was not loaded prior to calling _get_exposure_time_info")
            raise
